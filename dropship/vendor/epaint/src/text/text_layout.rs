...
struct Paragraph {
    ...
    /// dropship-ar: base direction is right-to-left (UAX#9 P2/P3)
    pub rtl: bool,
}
...
pub fn layout(fonts: &mut FontsImpl, pixels_per_point: f32, job: Arc<LayoutJob>) -> Galley {
    ...
    let (mut rows, mirrored_rows) =
        rows_from_paragraphs(paragraphs, &job, pixels_per_point, &mut elided);
    ...
    // dropship-ar: rtl paragraphs were wrapped in logical (mirrored) order so the
    // ragged row lands at the bottom and "…" at the logical end. flip each row
    // back to visual order now that wrapping and elision are done.
    for i in mirrored_rows {
        let row = Arc::make_mut(&mut rows[i].row);
        mirror_glyphs(&mut row.glyphs, row.size.x);
    }
...
fn layout_shaped_run(
    ...
    // rtl: clusters come in decreasing byte order, so the cluster being emitted
    // ends where the previously emitted one started.
    let mut cluster_end_byte: usize = run_text.len();
    ...
    for (info, pos) in iter::zip(glyph_buffer.glyph_infos(), glyph_buffer.glyph_positions()) {
        ...
        if is_new_cluster {
            if ctx.prev_cluster.is_some() {
                let cluster_bytes = if rtl {
                    cluster_start_byte..cluster_end_byte
                } else {
                ...
                cluster_end_byte = cluster_start_byte;
            }
...
fn layout_section(
    ...
    for (seg_idx, segment) in SplitOrWhole::new(section_text, job.break_on_newline).enumerate() {
        ...
        // dropship-ar: UAX#9 bidi. Resolve embedding levels once per segment, then
        // emit the level runs in visual (left-to-right) order, shaping each run in
        // its own direction. Pure-ASCII text skips all of this.
        // base direction: any rtl character makes the whole paragraph rtl (like
        // `dir="rtl"`), instead of UAX#9's first-strong rule, so arabic text that
        // happens to start with a latin word ("Esc ...") still wraps right-to-left.
        let has_rtl = !segment.is_ascii()
            && segment.chars().any(|c| {
                use unicode_bidi::BidiClass::{AL, R, RLE, RLI, RLO};
                matches!(unicode_bidi::bidi_class(c), R | AL | RLE | RLO | RLI)
            });
        let bidi_runs: Vec<(Range<usize>, bool)> = if !has_rtl {
            vec![(0..segment.len(), false)]
        } else {
            paragraph.rtl = true;
            let bidi = unicode_bidi::BidiInfo::new(segment, Some(unicode_bidi::Level::rtl()));
            let mut out = vec![];
        ...
        let num_bidi_runs = bidi_runs.len();
        for (bidi_idx, (bidi_range, rtl)) in bidi_runs.into_iter().enumerate() {
            let bidi_text = &segment[bidi_range];
            segment_into_runs(font, bidi_text, &mut runs);

            let num_runs = runs.len();
            for (run_idx, run) in runs.iter().enumerate() {
                let run_text = &bidi_text[run.byte_range.as_usize()];
                ...
                if bidi_idx == 0 && run_idx == 0 {
                    flags |= harfrust::BufferFlags::BEGINNING_OF_TEXT;
                }
                if bidi_idx + 1 == num_bidi_runs && run_idx + 1 == num_runs {
...
// Ignores the Y coordinate.
/// Reverse glyph order and flip x positions inside `[0, width]`.
/// Turns a visual-order rtl run into logical order and back.
fn mirror_glyphs(glyphs: &mut [Glyph], width: f32) {
    glyphs.reverse();
    for g in glyphs {
        g.pos.x = width - (g.pos.x + g.advance_width);
    }
}

/// Returns the rows plus the indices of rows that are still mirrored (rtl,
/// logical order) and must be flipped back by the caller.
fn rows_from_paragraphs(
    paragraphs: Vec<Paragraph>,
    job: &LayoutJob,
    pixels_per_point: f32,
    elided: &mut bool,
) -> (Vec<PlacedRow>, Vec<usize>) {
    let num_paragraphs = paragraphs.len();

    let mut rows = vec![];
    let mut mirrored_rows = vec![];
    ...
    for (i, paragraph) in paragraphs.into_iter().enumerate() {
        ...
        if paragraph.glyphs.is_empty() {
            ...
            } else if paragraph.rtl {
                // glyphs are in visual order; mirror to logical order so the greedy
                // line breaker fills rows from the start of the text.
                let mut mirrored = paragraph.clone();
                mirror_glyphs(&mut mirrored.glyphs, paragraph_width);
                let first_row = rows.len();
                line_break(&mirrored, job, &mut rows, elided);
                mirrored_rows.extend(first_row..rows.len());
